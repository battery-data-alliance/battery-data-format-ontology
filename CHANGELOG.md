...
### Changed
- Schedule-scoped term semantics generalized from "resets to zero" to "schedule-defined assignment", following confirmation from Arbin's MITS software team: schedules can reset the four accumulator fields to zero ("Set variable(s)", with increment/decrement options) but can also assign an arbitrary value or a formula result mid-test ("Set value"). Updated the four core term definitions, short descriptions, scopeNotes, and formulas (now Q(t) = Q(t_r) + integral, with t_r the most recent schedule-defined assignment), the four accumulator terms in the Arbin vendor module, and the CSVW schema descriptions. ADR 0001 amended.
